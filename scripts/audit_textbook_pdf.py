...
MARKDOWN_VIEW_RECOMPUTER = "scripts/build_textbook_markdown_view.py"
...
def build_markdown_view_method(source_sha256: str) -> dict[str, object]:
    """Declare which ledger fields are recomputable from the repository Markdown and which are frozen.

    The frozen fields are bound to the fixed baseline commit and the locally held
    source PDF.  That PDF is deliberately not tracked, so those fields can never
    be recomputed here; they must never be presented as live facts about the
    current Markdown.  Everything listed in ``recompute_scope`` is regenerated by
    ``build_textbook_markdown_view.py`` and enforced in CI.
    """
    return {
        "status": "enforced",
        "producer": "scripts/audit_textbook_pdf.py（需要源 PDF 与完整 Git 历史，不在 CI 运行）",
        "recomputer": MARKDOWN_VIEW_RECOMPUTER,
        "command": f"uv run python {MARKDOWN_VIEW_RECOMPUTER} --check",
        "recompute_scope": [
            "chapters[].normalized_markdown_chars",
            "chapters[].markdown_to_pdf_char_ratio",
            "chapters[].assets.markdown_figure_carriers",
            "chapters[].assets.markdown_table_titles",
            "chapters[].assets.missing_figure_carriers",
            "chapters[].assets.missing_table_titles",
            "chapters[].assets.table_structure_risks",
            "asset_inventory.figures[].markdown_carrier_lines",
            "asset_inventory.figures[].status",
            "asset_inventory.tables[].markdown_carrier_lines",
            "asset_inventory.tables[].status",
            "additional_unmarked_figures[].markdown_carrier_lines",
            "additional_unmarked_figures[].status",
            "baseline_evidence.records[].current_markdown_carrier_lines",
            "baseline_evidence.records[].current_status",
            ...
            "formula_status_items[].markdown_carrier_lines",
            "formula_status_items[].status",
            "additional_unmarked_formulas[].markdown_carrier_lines",
            "additional_unmarked_formulas[].status",
            "additional_unmarked_formulas[].nearest_heading_line",
            "additional_unmarked_formulas[].nearest_heading",
            "summary.markdown_unique_figure_carriers",
            "summary.markdown_unique_table_titles",
            "summary.missing_figure_carriers",
            "summary.missing_table_titles",
            "summary.table_structure_risks",
        ],
        "frozen_scope": [
            "source",
            "chapters[].physical_pages",
            "chapters[].normalized_pdf_chars",
            "chapters[].page_ngram_coverage",
            "chapters[].assets.pdf_figure_numbers",
            "chapters[].assets.pdf_table_numbers",
            "page_records[]",
            "asset_inventory.*[].pdf_reference_pages",
            "asset_inventory.*[].baseline_marker_ids",
            "baseline_evidence.*",
            "additional_unmarked_formulas[].baseline_*（含 baseline_needles 与 baseline_markdown_lines）",
            "historical_spliced_tables.baseline_* 与全部证据叙述",
            "key_content_debt",
            "debt_scope",
        ],
        "limitation": (
            f"上述冻结字段只对固定基线提交 {BASELINE_COMMIT} 与源 PDF {source_sha256} 成立；"
            "源 PDF 不入库，因此这些字段在正文改动后无法在本仓库内重算，"
            "也不得被当作对现行 Markdown 的实时结论。"
            f"可重算字段由 {MARKDOWN_VIEW_RECOMPUTER} --check 在 CI 中强制，"
            "任何正文改动都必须重算并提交账本。"
        ),
    }
...
def build_audit(pdf_path: Path, reviewed_at: str, width: int, manual_review_completed: bool) -> dict[str, object]:
    ...
    return {
        ...
        "field_definitions": {
            ...
            "current_needles": (
                "现行 canonical Markdown 中定位该公式载体所需的原文片段；"
                "build_textbook_markdown_view.py 据此从正文重算 markdown_carrier_lines。"
            ),
            "markdown_view": (
                "账本的 Markdown 侧（可从仓内正文重算，CI 强制）与 PDF 侧（冻结于不入库的源 PDF）边界声明。"
            ),
        },
        "source": {
        ...
        "method": {
            ...
            "markdown_view": build_markdown_view_method(sha256),
        },
        "manual_review": {
