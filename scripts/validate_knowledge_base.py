...
# the pypdf pin so it can refuse to regenerate the committed textbook audit
# with a different release.  These checks keep the copies from drifting.
PYPROJECT_PATH = ROOT / "pyproject.toml"
PYTHON_VERSION_PATH = ROOT / ".python-version"
UV_LOCK_PATH = ROOT / "uv.lock"
TEXTBOOK_AUDIT_SCRIPT_PATH = ROOT / "scripts" / "audit_textbook_pdf.py"
PYTHON_VERSION = "3.13"
REQUIRES_PYTHON = ">=3.13"
AUDIT_DEPENDENCY_GROUP = "audit"
# Non-default dependency groups, each pinning exactly one package.  Neither is
# installed for the main gate, which must keep running on the standard library.
DEPENDENCY_GROUPS = {AUDIT_DEPENDENCY_GROUP: "pypdf", "lint": "ruff"}
SCRIPT_PYPDF_VERSION_RE = re.compile(r'^PYPDF_VERSION = "(?P<version>[^"]+)"', re.MULTILINE)
...
UNTRACKED_ASSET_DEBT_RE = re.compile(r"待复核[：:].{0,120}(?:缺少|缺失)(?:表|图)|当前仅保留题面、选项与答案")
...
class Validator:
    ...
    def check_exam_manifest(self) -> None:
        ...
        if not isinstance(archive_policy, dict):
            ...
            if not isinstance(snapshot_commit, str) or not GIT_COMMIT_RE.fullmatch(snapshot_commit):
                self.error(
                    MANIFEST_PATH,
            ...
            if not isinstance(blob_template, str) or "{commit}" not in blob_template or "{path}" not in blob_template:
                self.error(
                    MANIFEST_PATH,
        ...
        if not isinstance(review_policy, dict):
            ...
            elif conflict_disposition.get("status") != ("retained_separate_source_limited"):
                self.error(
                    MANIFEST_PATH,
        ...
        if isinstance(review_policy, dict) and review_policy.get("canonical_exam_count") != len(exams):
            self.error(
                MANIFEST_PATH,
        ...
        if isinstance(review_policy, dict):
            conflict_disposition = review_policy.get("conflict_disposition")
            if (
                isinstance(conflict_disposition, dict)
                and conflict_disposition.get("same_name_conflict_count") != conflict_count
            ):
                self.error(
                    MANIFEST_PATH,
        ...
        if isinstance(review_policy, dict):
            ...
            expected_outcomes = {
                "complete_structural": sum(exam.get("completeness") == "complete_structural" for exam in exams),
                "needs_review": sum(exam.get("completeness") == "needs_review" for exam in exams),
                "partial": sum(exam.get("completeness") == "partial" for exam in exams),
                "medium_non_official_answers": sum(
                    exam.get("answer_confidence") == "medium_non_official" for exam in exams
                ),
                "low_non_official_answers": sum(exam.get("answer_confidence") == "low_non_official" for exam in exams),
        ...
        index_specs = (
            (
                ...
                {"period": 0, "subject": 1, "count": 3, "completeness": 4, "answer": 5, "notes": 9},
            ),
            (
                CLEAN_EXAM_INDEX_PATH,
                lambda preferred: Path(preferred).name,
                {"period": 0, "subject": 1, "count": 4, "completeness": 5, "answer": 6, "notes": 7},
            ),
        )
        for index_path, target_for, columns in index_specs:
            ...
            for exam in valid_exams:
                ...
                expected_cells = {
                    ...
                    "completeness": COMPLETENESS_INDEX_LABELS.get(str(exam.get("completeness")), ""),
                    "answer": ANSWER_CONFIDENCE_INDEX_LABELS.get(str(exam.get("answer_confidence")), ""),
                }
    ...
    def check_clean_exam_counts(self) -> None:
        ...
        for path in sorted(directory.glob("*综合知识*.md")):
            ...
            if unresolved_match:
                self.error(
                    path,
                    "contains an untracked unresolved asset phrase: " + unresolved_match.group(0),
                )
            ...
            for position, item in enumerate(group, start=1):
                ...
                if group_name == "additional_positions":
                    ...
                    if (
                        ...
                        or item.get("baseline_mapping") != "no_matching_legacy_marker_in_fixed_baseline"
                    ):
                        self.error(
                            EXAM_ASSET_AUDIT_PATH,
    ...
    def check_textbook_audit(self) -> None:
        ...
        if not isinstance(manual_review, dict):
            ...
            if not isinstance(explicit_pages, list) or explicit_pages != list(range(13, 721)):
                self.error(
                    TEXTBOOK_AUDIT_PATH,
            ...
            if not isinstance(components, dict):
                ...
                if components.get("additional_unmarked_formula_pages") != [61, 70, 113, 162, 170, 171]:
                    self.error(
                        TEXTBOOK_AUDIT_PATH,
                        "manual_review.scope_components.additional_unmarked_formula_pages is invalid",
                    )
                if components.get("full_page_visual_text_review_pages") != list(range(13, 721)):
                    self.error(
                        TEXTBOOK_AUDIT_PATH,
        ...
        if not isinstance(baseline_evidence, dict):
            ...
            if not isinstance(baseline_commit, str) or not GIT_COMMIT_RE.fullmatch(baseline_commit):
            ...
            for position, record in enumerate(records, start=1):
                ...
                if (
                    ...
                    or not all(isinstance(line, int) and line > 0 for line in carrier_lines)
                ):
                    self.error(
                        TEXTBOOK_AUDIT_PATH,
            ...
            if baseline_impact_counts != Counter({"critical_content_incomplete": 182, "graphic_detail_only": 98}):
                self.error(
                    TEXTBOOK_AUDIT_PATH,
        ...
        if not isinstance(additional_figures, list) or len(additional_figures) != 12:
            ...
            numbers = [item.get("number") if isinstance(item, dict) else None for item in additional_figures]
            if numbers != expected_additional_numbers:
                self.error(
                    TEXTBOOK_AUDIT_PATH,
                    "additional_unmarked_figures has unexpected numbers or order",
                )
            if any(not isinstance(item, dict) or item.get("baseline_marker_ids") != [] for item in additional_figures):
                self.error(
                    TEXTBOOK_AUDIT_PATH,
            ...
            for position, item in enumerate(additional_figures, start=1):
                ...
                if (
                    ...
                    or not item_id
                    or impact not in {"critical_content_incomplete", "graphic_detail_only"}
        ...
        if not isinstance(additional_formula_items, list) or len(additional_formula_items) != len(
            expected_additional_formulas
        ):
            self.error(
                TEXTBOOK_AUDIT_PATH,
            ...
            for position, (item, expected) in enumerate(
                ...
                if type(item.get("nearest_heading_line")) is not int or item["nearest_heading_line"] <= 0:
                    self.error(
                        TEXTBOOK_AUDIT_PATH,
                ...
                if (
                    ...
                    or not all(isinstance(needle, str) and needle.strip() for needle in baseline_needles)
                ):
                    self.error(
                        TEXTBOOK_AUDIT_PATH,
                ...
                if (
                    ...
                    or not all(type(line) is int and line > 0 for line in baseline_lines)
                ...
                if not isinstance(baseline_context, str) or not baseline_context.strip():
                    self.error(
                        TEXTBOOK_AUDIT_PATH,
                    ...
                    if not isinstance(supporting_evidence, dict):
                        ...
                        for field, expected_value in expected_supporting_evidence.items():
                            if supporting_evidence.get(field) != expected_value:
                                self.error(
                                    TEXTBOOK_AUDIT_PATH,
                                    f"{label}.supporting_evidence.{field} must be {expected_value!r}",
                                )
                item_id = item.get("id")
        ...
        if not isinstance(formula_items, list) or len(formula_items) != 2:
            ...
            topics = [item.get("topic") if isinstance(item, dict) else None for item in formula_items]
        ...
        if not isinstance(spliced, dict):
            ...
            if (
                ...
                or spliced.get("mapping_status") != "conservative_reconstruction_covers_reported_minimum"
            ...
            if not isinstance(candidates, list) or len(candidates) != 15:
                ...
                explicit_shifted = spliced.get("baseline_explicit_shifted_table_numbers_in_separate_8_item_category")
        ...
        if not isinstance(key_content, dict):
            ...
            if (
                key_content.get("derivation_status") != "conservative_reconstruction_from_baseline_markers"
            ...
            if (
                key_content.get("historical_conservative_reconstructed_positions") != 199
                or key_content.get("current_conservative_reconstructed_positions") != 208
            ):
                self.error(
                    TEXTBOOK_AUDIT_PATH,
                    "key_content_debt must record 199 historical and 208 current conservative positions",
                )
            additional_classification = key_content.get("additional_unmarked_figures_content_classification")
            if not isinstance(additional_classification, dict) or {
                ...
                "critical_content_incomplete": additional_classification.get("critical_content_incomplete"),
                "graphic_detail_only": additional_classification.get("graphic_detail_only"),
            } != {
                "status": "completed",
                "positions": 12,
            ...
            key_additional_critical = key_content.get("additional_critical_figure_position_ids")
            key_additional_detail = key_content.get("additional_detail_only_figure_position_ids")
            key_additional_formulas = key_content.get("additional_formula_position_ids")
            ...
            if (
                ...
                or not all(isinstance(item_id, str) and item_id for item_id in historical_position_ids)
            ...
            if (
                ...
                or not all(isinstance(item_id, str) and item_id for item_id in current_position_ids)
        ...
        for position, item in enumerate(page_records, start=1):
            ...
            expected_reasons = [reason for reason, pages in manual_reason_pages.items() if physical_page in pages]
        ...
        if page_ranges and (
            ...
            or any(current[1] + 1 != following[0] for current, following in itertools.pairwise(page_ranges))
        ):
    ...
    def check_python_toolchain(self) -> None:
        ...
        if not isinstance(groups, dict) or set(groups) != set(DEPENDENCY_GROUPS):
            ...
            return
        # Every group holds a single, exactly pinned requirement: the audit ledger
        # and the committed formatting are only reproducible at one version each.
        ...
        requirement = str(groups[AUDIT_DEPENDENCY_GROUP][0])
        script_text = TEXTBOOK_AUDIT_SCRIPT_PATH.read_text(encoding="utf-8-sig")
        script_match = SCRIPT_PYPDF_VERSION_RE.search(script_text)
        if script_match is None:
            self.error(TEXTBOOK_AUDIT_SCRIPT_PATH, "PYPDF_VERSION constant is missing")
            return
        script_version = script_match.group("version")
        if requirement != f"pypdf=={script_version}":
            self.error(
                PYPROJECT_PATH,
                f"dependency-groups.{AUDIT_DEPENDENCY_GROUP} pins {requirement!r} "
                f"but {self.relative(TEXTBOOK_AUDIT_SCRIPT_PATH)} pins "
                f"pypdf=={script_version}",
            )
        ...
        if locked.get("pypdf") != script_version:
            self.error(
                UV_LOCK_PATH,
                f"locks pypdf {locked.get('pypdf')!r} but the pin is {script_version!r}; run `uv lock`",
            )
