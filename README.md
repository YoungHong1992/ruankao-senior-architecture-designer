...
当前 36 份唯一试卷均有标准名首选稿并完成现有证据范围终审（36/36）；清洗目录另保留 2023 年下半年综合知识回忆版 A（65 题），因此共有 37 份试题正文。两个来源目录的 53 份原始版/补充版正文均已映射为备选，17 组同名冲突的终审处置是继续隔离而非强行混并。初始 116 个显式缺图/表/公式点位及后来发现的第 117 处均已有逐项审计记录；能够核验的内容已结构化恢复，6 个公开证据不足的视觉细节保留为显式限制。答案可信度仍为中·非官方 7 份、低·非官方 29 份，官方或高可信度 0 份。
...
scripts/build_exam_asset_audit.py   # 从固定基线重建 117 个真题点位记录
...
- PDF 中 284 个唯一教材图号与 59 个唯一表号均已建立独立 Markdown 载体。原 296 项资产债口径经逐页审计补发现 12 幅图，并从固定基线的压平单元格、PDF 表号页和现行管道表三重证据中保守重建 15 张互不重叠的拼栏表，修正下限为 309 项，现均有逐项记录。历史报告恰好选择的 14 个 ID 没有保存，因此账本只声明独立重建集合覆盖“至少 14 张”的数量下限，不冒充原 14/14 身份映射。结构化重绘不是原始版式复制。
