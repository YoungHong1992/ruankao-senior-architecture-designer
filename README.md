...
- PDF 中 284 个唯一教材图号与 59 个唯一表号均已建立独立 Markdown 载体。原 296 项资产债口径经逐页审计补发现 12 幅图，修正下限为 308 项；其中 294 项可从固定基线、PDF 和现行载体逐项追溯，历史口径中的 14 个拼栏位置未保存 ID 清单。当前 59 表全量结构扫描无同类残余，但不把这一现态检查冒充历史 14/14 身份映射。结构化重绘不是原始版式复制。
