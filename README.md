...
| [`01.系统架构设计师教材-清洗版`](01.系统架构设计师教材-清洗版/) | 前言 + 20 章 + 1 个索引 | 20 章、708 个正文物理页均有逐页文本覆盖记录；88 个低覆盖/新增资产/第 8、10 章页面完成声明式人工复核；284 个唯一图号和 59 个唯一表号均有独立 Markdown 载体；仍非出版社校正版 |
| [`02.历年真题`](02.历年真题/) | 33 份试题文件 | 本地原始材料；质量、题量和答案来源不一 |
| [`02.历年真题(补充)`](02.历年真题%28补充%29/) | 20 份试题文件 | 补充来源；与原始材料存在同名但内容不同的文件 |
| [`02.历年真题-清洗版`](02.历年真题-清洗版/) | 37 份试题正文 + 1 个索引 | 36 份 canonical 已完成现有证据范围终审，另有 1 份独立备选；结果为结构齐 24、待复核 8、部分 4，不代表官方认证 |

当前 36 份唯一试卷均有标准名首选稿并完成现有证据范围终审（36/36）；清洗目录另保留 2023 年下半年综合知识回忆版 A（65 题），因此共有 37 份试题正文。两个来源目录的 53 份原始版/补充版正文均已映射为备选，17 组同名冲突的终审处置是继续隔离而非强行混并。固定基线的 116 个显式缺图/表/公式点位，加终审中新发现的 2024 下第 73 题和 2025 上第 43 题，共 118 个位置均有逐项审计记录；能够核验的内容已结构化恢复，6 个公开证据不足的视觉细节保留为显式限制。答案可信度仍为中·非官方 7 份、低·非官方 29 份，官方或高可信度 0 份。
...
data/exam_asset_audit.json          # 118 个真题图表公式点位审计账本
data/textbook_audit.json            # 教材逐页覆盖与图号/表号审计结果
scripts/validate_knowledge_base.py  # 知识库质量检查入口
scripts/build_exam_asset_audit.py   # 从固定基线重建 118 个真题点位记录
...
- PDF 中 284 个唯一教材图号与 59 个唯一表号均已建立独立 Markdown 载体。原 296 项资产债口径经逐页审计补发现 12 幅图，并从固定基线的压平单元格、PDF 表号页和现行管道表三重证据中保守重建 15 张互不重叠的拼栏表，修正下限为 309 项，现均有逐项记录。固定基线还可按 A/B 风险语义反推出历史关键内容债下限 `174 + 2 + 8 + 14 = 198`；由于原 14 张拼栏表 ID 未保存，账本以 15 个独立候选形成 199 项高置信历史保守集合。12 幅后补图又经逐项 A/B 复核，4 幅属于内容关键、8 幅只影响图形细节，因此当前关键集合为 `174 + 4 + 2 + 8 + 15 = 203` 项；这些数字均不冒充原 198 项身份清单。结构化重绘不是原始版式复制。
