...
以下统计以各资料对应的索引与审计账本统计日期为准（真题/教材：2026-07-12；大纲：2026-09-06）。“已提取”或“已清洗”只表示已有对应 Markdown，不代表逐页、逐题完成权威校对。

| 目录 | 当前内容 | 状态 |
|---|---:|---|
| [`00.系统架构设计师考试大纲`](00.系统架构设计师考试大纲/) | 5 篇正文 + 1 个索引 | OCR/原始整理稿已纳入 |
| [`00.系统架构设计师考试大纲-清洗版`](00.系统架构设计师考试大纲-清洗版/) | 前言 + 5 篇正文 + 1 个索引 | 前言及第01—05章已完成三轮逐页视觉核对并修订（含 2026-09-06 独立复核）；账本记录 51 项 findings（含页码治理与时效性注） |
...
| [`02.历年真题-清洗版`](02.历年真题-清洗版/) | 37 份试题正文 + 1 个索引 | 36 份 canonical 已完成现有证据范围终审，另有 1 份独立备选；结果为结构齐 32、待复核 0、部分 4，不代表官方认证 |

当前 36 份唯一试卷均有标准名首选稿并完成现有证据范围终审（36/36）；清洗目录另保留 2023 年下半年综合知识回忆版 A（65 题），因此共有 37 份试题正文。两个来源目录的 53 份原始版/补充版正文均已映射为备选，17 组同名冲突的终审处置是继续隔离而非强行混并。固定基线的 116 个显式缺图/表/公式点位，加续审发现的 2024 上第 68、69 题、2024 下第 73 题和 2025 上第 43 题，共 120 个位置均有逐项审计记录；能够核验的内容已结构化恢复，6 个公开证据不足的视觉细节保留为显式限制。答案可信度仍为中·非官方 9 份、低·非官方 27 份，官方或高可信度 0 份。
...
- PDF 中 284 个唯一教材图号与 59 个唯一表号均已建立独立 Markdown 载体。原 296 项资产债口径经逐页审计补发现 12 幅图、5 个此前未登记的独立公式/数值主题，并从固定基线的压平单元格、PDF 表号页和现行管道表三重证据中保守重建 15 张互不重叠的拼栏表；连同历史已登记的 2 个公式，修正下限为 `272 + 12 + 7 + 8 + 15 = 314` 项，现均有逐项记录。固定基线仍可按 A/B 风险语义反推出历史关键内容债下限 `174 + 2 + 8 + 14 = 198`；由于原 14 张拼栏表 ID 未保存，账本以 15 个独立候选形成 199 项高置信历史保守集合。12 幅后补图经逐项 A/B 复核后有 4 幅属于内容关键，5 个新增公式/数值主题也属于内容关键，因此当前关键集合为 `174 + 4 + 2 + 5 + 8 + 15 = 208` 项；这些数字均不冒充原 198 项身份清单。结构化重绘不是原始版式复制。
