...
5. 教材正文变更后**先**运行 `uv run python scripts/build_textbook_markdown_view.py` 重算 `data/textbook_audit.json` 的 Markdown 侧字段（载体行号、状态、summary 计数），无源 PDF 也能跑且 CI 会复核；只有在本地合法持有源 PDF 时才进一步运行 `uv run --group audit python scripts/audit_textbook_pdf.py <PDF路径> --manual-review-completed` 重建 PDF 侧字段（带 `--check` 只校验不写回；不带 `--manual-review-completed` 重新生成会把人工复核状态写回 `pending`，校验器将拒绝）。真题图表点位变化同步 `data/exam_asset_audit.json`。
6. 跑完 CI 会跑的**全部四条检查**，全绿再提交：
   ...
   uv run python scripts/build_textbook_markdown_view.py --check  # 教材账本 Markdown 侧可重现；改过教材正文要先去掉 --check 重建
...
- `validate_knowledge_base.py`、`build_exam_asset_audit.py`、`build_outline_audit.py` 和 `build_textbook_markdown_view.py` 只能使用 Python 标准库，以保证质量门禁不依赖任何第三方包。
...
- pypdf 的版本在 `pyproject.toml`、`uv.lock` 和 `scripts/audit_textbook_pdf.py` 的 `PYPDF_VERSION` 三处必须一致（校验器会断言）；它决定 `data/textbook_audit.json` 的 PDF 侧字段能否复现，升级前须重新生成、重跑 `build_textbook_markdown_view.py` 并复核该账本。
