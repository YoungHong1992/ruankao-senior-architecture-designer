...
> **本章图表复核主源：** 本地文件 `本地pdf参考/1. 系统架构设计师教材（官方教程-）.pdf`，SHA-256 `EE45900F4622D71539980CFE1BDDBCD898FBA97CA13ADA6DF1CBDC215135C2F8`。经 `pdfinfo`/pypdf 定位，第13章为 PDF 第459～489页，对应印刷第451～481页；已将这31页逐页渲染并目视核对，核对日期为 `2026-07-12`。下列 Mermaid 与表格均为结构化重绘或转录，不是教材原图，不复制扫描页、水印或原图几何版式。
...
在一些层次系统中，除了一些精心挑选的输出函数外，内部的层接口只对相邻的层可见。连接件通过决定层间如何交互的协议来定义，拓扑约束包括对相邻层间交互的约束。由于每一层最多只影响两层，同时只要给相邻层提供相同的接口，允许每层用不同的方法实现，同样为软件重用提供了强大的支持。

软件层次式体系结构是最通用的架构，也被叫作N 层架构模式 (n-tier architecture pattern)。

这也是Java EE (也称为J2EE) 应用经常采用的标准模式。这种架构模式非常适合传统的IT通信和组织结构，很自然地成为大部分应用的第一架构选择。在分层次体系结构中的组件被划分成几个层，每个层代表应用的一个功能，都有自己特定的角色和职能。分层架构本身没有规定要分成多少层，大部分的应用会分成表现层(或称为展示层)、中间层(或称为业务层)、数据访问层(或称为持久层)和数据层。其结构见图 13-1所示。
...
```mermaid
flowchart TB
    R["Request"] --> P["表现层<br/>组件 × 4<br/>封闭"]
    P --> M["中间层<br/>组件 × 4<br/>封闭"]
    M --> A["访问层<br/>组件 × 4<br/>封闭"]
    A --> D[("数据层<br/>数据存储 × 4<br/>封闭")]
```

> **结构化重绘说明：** 基于上述本地 PDF（SHA-256 `EE45900F4622D71539980CFE1BDDBCD898FBA97CA13ADA6DF1CBDC215135C2F8`）PDF 第460页 / 印刷第452页，核对日期 `2026-07-12`；不是教材原图。原图每层横向画出4个示例组件并标为“封闭”，这里聚合为“×4”，纵向箭头保留请求依次穿过四层的方向。
...
```mermaid
flowchart LR
    V["View（视图）<br/>显示模型状态<br/>接受数据更新请求<br/>把用户输入数据传给控制器"]
    C["Controller（控制器）<br/>接受用户请求<br/>调用模型响应用户请求<br/>选择视图显示响应结果"]
    M["Model（模型）<br/>代表应用程序状态<br/>响应状态查询<br/>处理业务流程<br/>通知视图业务状态更新"]
...
> **结构化重绘说明：** 基于上述本地 PDF（SHA-256 `EE45900F4622D71539980CFE1BDDBCD898FBA97CA13ADA6DF1CBDC215135C2F8`）PDF 第461页 / 印刷第453页，核对日期 `2026-07-12`；不是教材原图。实线转录原图图例中的方法调用，虚线转录事件；节点内职责与五条具名关系均按原图保存。
...
MVP(Model-View-Presenter) 模式中，Model 提供数据，View 负责显示，Controller/Presenter 负责逻辑处理。MVP 是从经典的 MVC 模式演变而来的，它们的基本思想有相通之处：Controller/Presenter 负责逻辑处理，Model 提供数据，View 负责显示。当然，MVP 与 MVC 也有一些显著区别。MVC 模式中元素之间“混乱”的交互，主要体现在允许 View 和 Model 直接进行“交流”，这在 MVP 模式中是不允许的。在 MVP 中，View 并不直接使用 Model，它们之间的通信通过 Presenter（MVC 中的 Controller）进行，所有交互都发生在 Presenter 内部；而在 MVC 中，View 会直接从 Model 中读取数据，而不是通过 Controller。
...
flowchart LR
    ...
    P -- "3: updates the model<br/>(use View as a change's store)" --> M
    M -- "4: gets updated data" --> P
    P -. "5: notification of<br/>Model changes" .-> V
    P -- "6: rendering" --> V
    V -- "7: rendered output" --> U
```

> **结构化重绘说明：** 基于上述本地 PDF（SHA-256 `EE45900F4622D71539980CFE1BDDBCD898FBA97CA13ADA6DF1CBDC215135C2F8`）PDF 第462页 / 印刷第454页，核对日期 `2026-07-12`；不是教材原图。保留原图英文标签、1～7编号和方向；第2、5步使用虚线，对应原图的通知关系。
...
> **结构化重绘说明：** 基于上述本地 PDF（SHA-256 `EE45900F4622D71539980CFE1BDDBCD898FBA97CA13ADA6DF1CBDC215135C2F8`）PDF 第463页 / 印刷第455页，核对日期 `2026-07-12`；不是教材原图。保留 View→ViewModel 的 DOM 事件、ViewModel→View 的 DataBinding，以及 ViewModel 与 Model 间原图未命名的双向关系。
...
```mermaid
flowchart LR
    U["Users"] <--> UIC["UI Components<br/>(UIC)"]
    DAC["Data Access Components<br/>(DAC)"] <--> DS[("Data Sources")]
    SG["Service Gateways<br/>(SG)"] <--> S["Sources"]
```

> **结构化转录/重绘说明：** 基于上述本地 PDF（SHA-256 `EE45900F4622D71539980CFE1BDDBCD898FBA97CA13ADA6DF1CBDC215135C2F8`）PDF 第465页 / 印刷第457页，核对日期 `2026-07-12`；不是教材原图。表格保存原图分层及三个纵向关注点；Mermaid 只画原图明确给出的三组双向箭头，不为相邻方框补造调用关系。
...
> **结构化重绘说明：** 基于上述本地 PDF（SHA-256 `EE45900F4622D71539980CFE1BDDBCD898FBA97CA13ADA6DF1CBDC215135C2F8`）PDF 第466页 / 印刷第458页，核对日期 `2026-07-12`；不是教材原图。七个模块及单向、双向箭头均按原图转录，自动布局不复刻原图上下左右位置。
...
定义FacadeManager组件时必须为其配置所需要的DAO组件，配置信息表示BaseManager继承刚才配置的事务代理模板，并且由容器给BaseManager注入DAO组件，即BaseDAOHibernate。`target` 则是TransactionProxyFactoryBean需要指定的属性；TransactionProxyFactoryBean负责为某个bean实例生成代理，代理必须有一个目标，`target` 属性用于指定该目标。
...
flowchart TB
    PDT["过程定义工具"] -- "接口1" --> CORE
    ADMIN["管理与监视工具"] <-->|"接口5"| CORE
    CLIENT["工作流客户端应用"] <-->|"接口2"| CORE
    APP["相关应用"] <-->|"接口3"| CORE
    OTHER["其他工作流设置服务器<br/>工作流引擎 × N"] <-->|"接口4"| CORE

    subgraph COREBOX["工作流API和交互形式"]
        CORE["工作流设置服务器<br/>工作流引擎 × N"]
    end
```

> **结构化重绘说明：** 基于上述本地 PDF（SHA-256 `EE45900F4622D71539980CFE1BDDBCD898FBA97CA13ADA6DF1CBDC215135C2F8`）PDF 第468页 / 印刷第460页，核对日期 `2026-07-12`；不是教材原图。接口1为过程定义工具导入，接口2～5保留原图双向箭头；“×N”表示原图叠放的多个工作流引擎，不限定具体数量。
...
flowchart LR
    subgraph DS["DataSet"]
        UC(["UniqueConstraint"]) --> T["Product DataTable"]
        NOTE["将 ProductID 定义为<br/>Product DataTable 中的主键"] --- UC
    end
```

> **结构化重绘说明：** 基于上述本地 PDF（SHA-256 `EE45900F4622D71539980CFE1BDDBCD898FBA97CA13ADA6DF1CBDC215135C2F8`）PDF 第469页 / 印刷第461页，核对日期 `2026-07-12`；不是教材原图。保留 DataSet 容器、Product DataTable、UniqueConstraint 及 ProductID 主键约束，省略原图无字段名的空白表格行。
...
flowchart TB
    subgraph DS["DataSet"]
        T1["Product DataTable<br/>（原图标签）"]
        T2["Product DataTable<br/>（原图标签）"]
        R(["关系"]) <-->|"关联 Order 和<br/>OrderDetails DataTable"| T1
        R <--> T2
        U1(["UniqueConstraint"]) --> T1
        U2(["UniqueConstraint"]) --> T2
        N1["将 ProductID 定义为<br/>上表中的主键"] --- U1
        N2["将 OrderID 和 ProductID 定义为<br/>OrderDetails DataTable 中的复合主键"] --- U2
    end
```

> **结构化重绘说明：** 基于上述本地 PDF（SHA-256 `EE45900F4622D71539980CFE1BDDBCD898FBA97CA13ADA6DF1CBDC215135C2F8`）PDF 第470页 / 印刷第462页，核对日期 `2026-07-12`；不是教材原图。官方原图上下两个表框都印作 `Product DataTable`，且上方约束说明与紧邻正文所述 Order/OrderDetails 语义不一致；这里按原图保留表框标签并显式标注，不静默改写。关系、两个约束对象和下表复合主键说明按原图转录。
...
flowchart TB
    ...
    VIEW --> REQ --> BIZ --> DAOI --> DAOX --> DB
```

> **结构化重绘说明：** 基于上述本地 PDF（SHA-256 `EE45900F4622D71539980CFE1BDDBCD898FBA97CA13ADA6DF1CBDC215135C2F8`）PDF 第471页 / 印刷第463页，核对日期 `2026-07-12`；不是教材原图。保留表示层四类客户端、业务需求、业务容器、DAO界面、DAO执行和数据库的纵向位置；原图业务容器写作截断的 `DomainM`，结合紧邻正文明确给出的 `Domain Model—Service—Control` 转录为完整名称。

从图 13-10 中可以看到，业务层采用业务容器(Business Container)的方式存在于整个系统中。采用此方式可以大大降低业务层和相邻各层的耦合，表示层代码只需要将业务参数传递给业务容器，而不需要业务层多余的干预。如此一来，可以有效地防止业务层代码渗透到表示层。
...
2. Control作为服务控制器，根据Domain Model的状态和相关参数决定Service之间的执行和切换。
...
sequenceDiagram
    ...
    participant B as 进程边界
    participant D as :aClient
    C->>D: GetTitle()
    D-->>C: 返回值
    C->>D: GetFirstName()
    D-->>C: 返回值
...
> **结构化重绘说明：** 基于上述本地 PDF（SHA-256 `EE45900F4622D71539980CFE1BDDBCD898FBA97CA13ADA6DF1CBDC215135C2F8`）PDF 第472页 / 印刷第464页，核对日期 `2026-07-12`；不是教材原图。四次读取及每次返回均跨越原图标出的进程边界；`返回值`是对原图无文字返回箭头的说明，不代表固定字段值。
...
> **结构化重绘说明：** 基于上述本地 PDF（SHA-256 `EE45900F4622D71539980CFE1BDDBCD898FBA97CA13ADA6DF1CBDC215135C2F8`）PDF 第472页 / 印刷第464页，核对日期 `2026-07-12`；不是教材原图。三个 Web 应用、五个 DAO 及其到两个数据库和一个 ERP 系统的五组双向访问关系均按原图转录；编号仅用于区分原图中的重复实例。
...
> **结构化重绘说明：** 基于上述本地 PDF（SHA-256 `EE45900F4622D71539980CFE1BDDBCD898FBA97CA13ADA6DF1CBDC215135C2F8`）PDF 第473页 / 印刷第465页，核对日期 `2026-07-12`；不是教材原图。保留客户端跨进程向服务器获取 DTO、服务器创建 DTO，以及客户端随后在本进程内读取四项数据的顺序；`返回 CustomerDTO`和`返回值`用于文字化原图未标文字的返回箭头。
...
在应用程序的设计中，数据库的访问是非常重要的，数据库的访问需要良好的封装性和可维护性。在.NET中，对于微软自家的SQL Server和其他数据库（支持OleDb），采用不同的访问方法，这些类分别分布于System.Data.SqlClient和System.Data.OleDb命名空间中。微软后来又推出了专门用于访问Oracle数据库的类库。我们希望在编写应用系统时，不受这些类差异的影响，尽量做到数据库无关。
...
现在已经完成了所要的功能，下面需要创建一个Factory类，实现自动数据库切换管理。这个类的主要功能就是根据数据库类型，返回适当的数据库操作类。
...
当数据库发生变化时，只需要修改PersistenceProperty的值，客户端不会感觉到变化，也不用去关心。这样，实现了良好的封装性。当然，前提是编写程序时没有用到特定数据库的特性，例如SQL Server的专用函数。
...
通过Cache的实现，能够对性能进行调优。ORM实现了实际数据存储和业务层之间的隔离，能够对每一层进行单独跟踪，增加了性能优化的可能。

Hibernate是一个开放源代码的对象关系映射框架，它对JDBC进行了轻量级的对象封装，使Java程序员可以使用面向对象的编程思维来操纵数据库。它不仅提供了从Java类到数据表之间的映射，还提供了数据查询和恢复机制。相对于使用JDBC和SQL手工操作数据库，Hibernate可以大大减少操作数据库的工作量。另外，Hibernate可以利用代理模式简化类的载入过程，这将大大减少利用Hibernate Query Language(HQL)从数据库提取数据的代码编写量。Hibernate可以与多种Web服务器或应用服务器良好集成，并支持几乎所有流行的数据库服务器。

Hibernate技术本质上是一个提供数据库服务的中间件，其架构如图 13-14 所示。

> 图 13-14 Hibernate架构图

<div align="center">
<svg xmlns="http://www.w3.org/2000/svg" width="700" height="440" viewBox="0 0 700 440" role="img" aria-labelledby="fig13-14-title fig13-14-desc" style="max-width: 100%; height: auto;">
  <title id="fig13-14-title">图 13-14 Hibernate架构图结构化重绘</title>
  <desc id="fig13-14-desc">应用层在上，Hibernate层居中，数据库层在下；持久对象横跨应用层与Hibernate层的边界，Hibernate属性和XML映射位于Hibernate层内部。原图没有箭头。</desc>
  <g font-family="Microsoft YaHei, Noto Sans CJK SC, sans-serif" fill="white" stroke="#2f9fbd" stroke-width="1.5">
    <rect x="100" y="30" width="500" height="120"/>
    <rect x="100" y="170" width="500" height="160"/>
    <rect x="100" y="350" width="500" height="60"/>
    <rect x="130" y="235" width="190" height="70"/>
    <rect x="380" y="235" width="190" height="70"/>
    <rect x="180" y="105" width="340" height="80"/>
  </g>
  <g font-family="Microsoft YaHei, Noto Sans CJK SC, sans-serif" fill="#222" text-anchor="middle">
    <text x="350" y="70" font-size="23">应用</text>
    <text x="350" y="218" font-size="23">Hibernate</text>
    <text x="225" y="277" font-size="21">Hibernate 属性</text>
    <text x="475" y="277" font-size="21">XML 映射</text>
    <text x="350" y="155" font-size="23">持久对象</text>
    <text x="350" y="388" font-size="23">数据库</text>
  </g>
</svg>
</div>

> **结构化重绘说明：** 基于上述本地 PDF（SHA-256 `EE45900F4622D71539980CFE1BDDBCD898FBA97CA13ADA6DF1CBDC215135C2F8`）PDF 第477页 / 印刷第469页，核对日期 `2026-07-12`；不是教材原图。源图没有箭头，因而本稿不再把层次误写成“应用 → 持久对象 → Hibernate → 数据库”的调用方向。SVG 保留“持久对象”同时跨越应用层底边和 Hibernate 层顶边的几何语义，并保留 Hibernate 属性、XML 映射在 Hibernate 层内及数据库独立位于下层的边界关系；颜色、字体和绝对尺寸仅为可读性重绘。

图 13-14 显示了Hibernate配置文件（如hibernate.properties）的工作原理：它利用数据库以及XML Mapping等其他配置，为应用程序提供数据持久化服务。
...
对于目前关注和应用较多的RFID 网络来说，附着在设备上的RFID标签和用来识别 RFID信息的扫描仪、感应器都属于物联网的感知层。在这一类物联网中被检测的信息就是RFID标签的内容，现在的电子不停车收费系统 (Electronic Toll Collection,ETC)、超市仓储管理系统、飞机场的行李自动分类系统等都用到了这个层次的设备。
...
应用层解决的是信息处理和人机交互的问题。网络层传输而来的数据在这一层进入各类信息系统进行处理，并通过各种设备与人进行交互。这一层也可按形态直观地划分为两个子层。
...
PetShop是一个范例，微软用它来展示.NET企业系统开发的能力。PetShop随着版本的不断更新，至基于.NET 2.0的PetShop 4.0为止，整个设计逐渐变得成熟而优雅，有很多可以借鉴之处。PetShop是一个小型项目，系统架构与代码都比较简单，却也凸显了许多颇有价值的设计与开发理念。
...
```mermaid
flowchart TB
    UI["表示层 / IIS<br/>ASP.NET Web Forms"]
    BIZ["业务逻辑层 / CLR<br/>C#业务构件"]
    DAL["数据访问层 / CLR<br/>数据访问构件<br/>ADO.NET"]
    AP[("账目与产品")]
    ORDER[("订单")]
...
```mermaid
flowchart TB
    UI["表示层<br/>ASP.NET Web Forms"]
    BIZ["业务逻辑层<br/>C#构件<br/>企业服务器"]
    DAAB["DAAB"]
    DB1[("SQL Server 2000<br/>账目、产品")]
    DB2[("SQL Server 2000<br/>订单")]
...
```mermaid
flowchart LR
    F["DALFactory<br/>创建DAL对象的工厂"] -.-> I["IDAL<br/>数据访问层的接口程序集"]
    I -.-> M["Model<br/>数据访问层使用的持久对象<br/>只有属性，没有行为"]
    S["SQLServerDAL<br/>访问MS SQL Server数据库<br/>实现IDAL接口"] -.-> I
    O["OracleDAL<br/>访问Oracle数据库<br/>实现IDAL接口"] -.-> I
```
...
```mermaid
flowchart TB
    B["BLL<br/>业务逻辑层<br/>核心业务代码"]
    S["IBLLStrategy<br/>业务逻辑层对业务策略的抽象封装"]
    F["DALFactory"]
    I["IDAL"]
    M["Model"]
...
flowchart TB
    subgraph APP["应用层"]
        T["云平台小票服务<br/>小票实时推送<br/>历史小票查询<br/>会员积分查询"]
        D["云平台数据服务<br/>可视化经营数据管理<br/>用户精准推荐<br/>区域畅销品排名"]
    end
    subgraph NET["网络层"]
        N["电子小票云平台基础服务<br/>数据传输与处理｜数据存储｜智能硬件设备管理<br/>商家管理｜会员管理｜商品管理"]
    end
    subgraph SENSOR["感知层"]
        H["小票智能硬件（采集收银机待打印的购物小票）<br/>系统设计｜驱动程序｜控制程序"]
    end
...
```mermaid
flowchart LR
    POS["商家收银机"] -- "USB转串口" --> MCU["STM32控制器"]
    FONT["字模存储Flash"] -- "SPI" --> MCU
    MCU -- "GPIO" --> LCD["TFTLCD屏"]
    MCU <-->|"串口"| WIRELESS["无线模块"]
    WIRELESS ---|"Wi-Fi/GPRS"| CLOUD["电子小票云平台"]
    CLOUD --- WECHAT["微信公众平台"]
    CLOUD --- PC["商家PC终端"]
    WECHAT --- PHONE["消费者智能手机"]
```
